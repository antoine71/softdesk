...
class IssuePermissionsTest(TestCase):
    ...
    def test_create_contributor_no_assignee(self):
        ...
        self.assertEquals(response.status_code, 201)
        data = json.loads(response.content)
        self.assertEquals(data['assignee'], 'user1')
        self.assertEquals(Issue.objects.filter(project=1).count(), 3)
    ...
    def test_update_author(self):
        ...
        self.assertContains(response, 'title_update')
    ...
    def test_delete_author(self):
        ...
        self.assertEquals(Issue.objects.filter(project=1).count(), 1)
        self.assertEquals(Comment.objects.filter(issue=1).count(), 0)
